...
def run_gpt(text="面壁者", max_new_tokens=48):
    # 加载已经训练好的SentencePiece模型
    model_path = 'C:\\my_gpt_cmd\\my_bpe_model.model'  # 这里填写你的模型文件路径
    sp = spm.SentencePieceProcessor(model_file=model_path)
    x = torch.tensor([sp.Encode(text)])

    config = GPTconfig()
    clone_model = GPTmodel(config)
    clone_model.load_state_dict(torch.load("C:\\my_gpt_cmd\\my_gpt_model.pt"))
    clone_model.eval()
    out_tensor = clone_model.genarate(x, max_new_tokens)
    out = sp.Decode(out_tensor.tolist()[0])
    out = "\n"+out+"\n"+"-------------------------------------------------------------------"
    return out

def txt_save(path, text):
    with open(path, 'w', encoding='ANSI') as f:
        f.write(text)
        print("保存成功")

if __name__ == '__main__':
    data = csv_send("C:\\my_gpt_cmd\\send.csv")
    text = data[0][0]
    num = int(data[0][1])
    out = run_gpt(text, num)
    print(out)
    txt_save("C:\\my_gpt_cmd\\save.txt", out)
